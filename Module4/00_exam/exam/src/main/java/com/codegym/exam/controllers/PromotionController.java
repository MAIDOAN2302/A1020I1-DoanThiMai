...
public class PromotionController {
        ...
        if (!bindingResult.hasFieldErrors("startDate")) {
            try {
                startDate = LocalDate.of(
                        Integer.parseInt(promotion.getStartDate().substring(6, 10)),
                        Integer.parseInt(promotion.getStartDate().substring(3, 5)),
                        Integer.parseInt(promotion.getStartDate().substring(0, 2)));
            } catch (Exception e) {
                bindingResult.addError(new FieldError("promotion", "startDate",
        ...
        if (!bindingResult.hasFieldErrors("endDate")) {
            try {
                endDate = LocalDate.of(
                        Integer.parseInt(promotion.getEndDate().substring(6, 10)),
                        Integer.parseInt(promotion.getEndDate().substring(3, 5)),
                        Integer.parseInt(promotion.getEndDate().substring(0, 2)));
            } catch (Exception e) {
                bindingResult.addError(new FieldError("promotion", "endDate",
    ...
    public String showFormUpdateCustomer(@PathVariable int id, Model model) {
    ...
    @PostMapping("/update")
    public String updateCustomer(@Valid @ModelAttribute Promotion promotion, BindingResult bindingResult,
