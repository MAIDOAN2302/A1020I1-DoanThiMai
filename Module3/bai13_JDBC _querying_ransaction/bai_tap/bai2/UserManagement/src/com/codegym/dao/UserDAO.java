...
public class UserDAO implements IUserDAO {
    ...
    private static final String SELECT_ALL_USERS_BY_KEYWORD = "select * from users where country like ? order by name";
    ...
    public boolean deleteUserStore(int id) throws SQLException {
        boolean rowDeleted;
        String query = "{CALL sp_delete_users(?)}";
    ...
    public boolean updateUserStore(User user) throws SQLException {
        boolean rowUpdated;
        String query = "{CALL sp_edit_users(?, ?, ?, ?)}";
        try (Connection connection = getConnection(); CallableStatement statement = connection.prepareCall(query);) {
...
//            preparedStatement.setString(1, user.getName());
    ...
    public List<User> selectAllUsersByKeyword(String keyword) {
        ...
        try (Connection connection = getConnection();
             ...
             // Step 2:Create a statement using connection object
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ALL_USERS_BY_KEYWORD);) {
